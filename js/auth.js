...
async function submitAccessRequest(){
  ...
  if(!email){showToast('⚠️ Isi email dulu');return;}
  try{
    const existing=await db.collection('access_requests').where('email','==',email).where('status','==','pending').get();
    if(!existing.empty){showToast('⚠️ Permintaan kamu masih pending, tunggu approve master dulu');return;}
