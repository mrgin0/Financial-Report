...
const PAGE_FILES = {
  ...
  hutang:'hutang', exp:'pengeluaran', inc:'pemasukan', admin:'admin'
};
...
// Dipanggil auth.js sesaat setelah user berhasil login / baru daftar —
// load fragment Dashboard duluan (halaman default tiap kali masuk), baru fetch data & init()
...
function reRender(){
  ...
  if(p==='page-admin')renderAdminPage();
}
...
async function nav(id,el){
  ...
  if(id==='admin'&&!checkIsMaster()){showToast('⛔ Cuma master yang bisa buka halaman ini');return;}
  try{
...
// ══════════════════════════════════════════════════════════════
// BOOT — cuma nyiapin UI shell doang. Load data (bootApp) dipicu auth.js
// setelah onAuthStateChanged tau user login atau enggak.
// ══════════════════════════════════════════════════════════════
